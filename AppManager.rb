...
class App
  ...
  def self.open foldername
    # looks for app in database
    if appIndex = Database.search(foldername, true)
      # if found
      db = Database.read
      pp db
      appDir = db["webApps"][appIndex]["dir"]
      Sublime.start appDir
      Server.start appDir
    else
      puts 'app not found'.yellow
    end
  end
  ...
  def self.create title, foldername, type = "default"
      ...
      if Dir.exist? PROJECTS_DIR
        ...
        # starts browser-sync server and sublime text on app directory
        Sublime.start webApp["dir"]
        Server.start webApp["dir"]
